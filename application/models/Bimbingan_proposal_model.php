...
private function _get_datatables_query($id,$dosen_id) {
	$this->db->select('bimbingan_proposal.*,role.role as oleh,users.username as nim');
	$this->db->from('bimbingan_proposal');
	$this->db->join('role', 'bimbingan_proposal.role_id = role.id AND bimbingan_proposal.mahasiswa_id = '.$id.' AND bimbingan_proposal.dosen_id='.$dosen_id);
	$this->db->join('users', 'bimbingan_proposal.mahasiswa_id = users.user_id');
	$i = 0;
