	...
	<div class="col-md-12">
			...
			<div class="col-md-12">
				  ...
				  <div class="align-self-center">
					<?php if ($absen->pembimbing_1 == 'hadir' && $pembimbing[0]->role == 'Pembimbing 1 Proposal') : ?>
						<button data-toggle="modal" data-target="#nilai_proposal" class="btn btn-success text-white" data-aos="zoom-in">berikan nilai</button>
						<button data-toggle="modal" data-target="#modal-catatan_proposal" class="btn btn-success text-white" data-aos="zoom-in">Catatan</button>
						<?php else: ?>
							<?php if ($absen->pembimbing_2 == 'hadir' && $pembimbing[0]->role == 'Pembimbing 2 Proposal') : ?>
								<button data-toggle="modal" data-target="#nilai_proposal" class="btn btn-success text-white" data-aos="zoom-in">berikan nilai</button>
							<?php else: ?>
		...
		<?php else : ?>
			<?php if ($pembimbing[0]->role == "Pembimbing 1 Proposal") : ?>
				<div class="row" data-aos="fade-up">
				 ...
				 <?php if ($proposal->status_p1 == 'belum ACC') : ?>
					<button class="btn btn-primary mt-1" onclick="acc_proposal(<?= $proposal->id ?>,'status_p1')">ACC Proposal</button>
					<?php else: ?>
						<button class="btn btn-danger mt-1" onclick="batal_proposal(<?= $proposal->id ?>,'status_p1')">batalkan ACC Proposal</button>
				 <?php endif ?>
				 </div>
				  </div>
				  </div>
				  ...
				  <p class="text-center mt-2"><?= $proposal->status_p2 ?></p>
				  </div>
				  </div>
					</div>
			  </div>
			<?php else: ?>
				<div class="row" data-aos="fade-up">
				  ...
				  <h3 class="mb-0 text-center">Pembimbing 2</h3>
				 	<div class="button-acc">
					 <?php if ($proposal->status_p2 == 'belum ACC') : ?>
					<button class="btn btn-primary mt-1" onclick="acc_proposal(<?= $proposal->id ?>,'status_p2')">ACC Proposal</button>
					<?php else: ?>
						<button class="btn btn-danger mt-1" onclick="batal_proposal(<?= $proposal->id ?>,'status_p2')">batalkan Seminar Proposal</button>
					<?php endif ?>
					 </div>
				  </div>
				  </div>
				  ...
				  <div class="align-self-center">
				  <h3 class="mb-0 text-center">Pembimbing 1</h3>
					<p class="text-center mt-2"> <?= $proposal->status_p1 ?></p>
				  </div>
				  </div>
					</div>
			  </div>
			...
			<div class="col-md-12">
				...
				<?php if ($proposal->file) : ?>
					<a href="<?= base_url("mahasiswa/download_proposal/".$proposal->file) ?>" class="btn btn-success text-white">Unduh Proposal Fix</a>
					<?php else: ?>
						<button class="btn btn-success text-white" disabled>Unduh Proposal Fix</button>
				<?php endif ?>
				  </div>
				  </div>
					</div>
...
<button type="button" id="btn-tambahbp" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-bimbingan" data-id="<?= $mahasiswa->user_id ?>" data-role_id="<?= $r->id ?>" data-username="<?= $mahasiswa->username ?>" data-role="<?= str_replace(" ","_",$pembimbing[0]->role) ?>">
  <i class="fas fa-users"></i> Tambah Topik 
            ...
            <div class=" py-4  table-responsive">
              <table class="table table-flush" id="datatable-logbimbingandosen1">
            ...
            <div class=" py-4  table-responsive">
              <table class="table table-flush" id="datatable-logbimbingandosen2">
                <thead class="thead-light">
                  <tr>
					<th>No</th>
                    <th>Tanggal</th>
...
<div class="modal fade" id="hapus_bp" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-dialog">
	...
	<div class="form-group">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="bimbingan(<?= userdata()->user_id ?>)">Kirim</button>
        </form>
      </div>
    </div>
  </div>
  ...
  <div class="modal-dialog">
	...
	<div class="form-group">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="button" id="btn-editbp" class="btn btn-primary" onclick="editlogbp()">Save</button>
        </form>
      </div>
    </div>
  </div>
...
<!-- delete bimbingan proposal -->
<div class="modal fade" id="nilai_proposal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
    ...
    <input type="text" class="form-control" id="input-nilai" aria-describedby="emailHelp" placeholder="0">
				...
				<div class="row">
					...
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
        	<button type="button" class="btn btn-primary" onclick="nilai_proposal()" >kirim</button>
					</div>
				</div>	
    </div>
  </div>
...
<!-- Modal -->
<div class="modal fade" id="modal-catatan_proposal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
	<form action="<?= base_url("dosen/catatan_proposal/".$mahasiswa->user_id.'/'.$pembimbing[0]->user_id) ?>" method="post">
	<div class="form-group">
